...
#la posso mettere in un file functions e chiamarla al momento opportuno dallo script principale
# o dalla macchina a stati
...
def face_rec() :
    global people, tts2, predicates, flag
    counter=0
    predicates = []
    op = True
    #reader_flag=False
    ...
    #predicates=create_predicate (mypredicates, people)
    #predicates=queue1.get()
    #reader_flag=True
    #print (reader_flag)
...
class StateMachine:
    ...
    def state_zero (self):
        ...
        thread1=SensorsThread("face_recognition")
        thread1.start()
        ...
        for action in actions:
            ...
            if a in actions.values() :
                ...
                if a == actions.get('action5'):
                    global running
                    mypredicates=["isIn", "isSad", "isHappy"]
                    predicates=create_predicate(mypredicates,people)
                    # while queue1.empty():
                    #     time.sleep(0.1)
                    #predicates=queue1.get()
